...
if(!empty($_POST)){
	...
	// Vérification de l'hotel sélectionné
	// if ((!is_numeric($post['input_pays'])) || ($post['input_pays'] < 1 || $post['input_pays'] > 241)) {
	// 	$errors[] = 'Vous devez noter la difficulté de la recette';
	// }


	// Vérification de la durée sélectionnée
	...
	if(!empty($post['input_description'])){
		...
		if(strlen($post['input_description']) < 100 ){
			$errors[] = 'La description du séjour doit comporter au moins 200 caractères.';
		}
	} else {
	...
	if($_FILES['input_picture']['error'] == UPLOAD_ERR_OK){
		...
		$errors[] = 'Le téléchargement  n\'a été faite';
	}
	...
	if(count($errors) == 0){
		...
		$requete->bindValue(':actived_param', $post['input_actived']);
	        			...
	        			<div class="row mb-3">
	                		...
	                		<div class="col-6">
								...
								<select id="term" name="input_term" class="form-control">
									<option selected>veuilliez selectionner une durée</option>
